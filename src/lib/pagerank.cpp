    ...
    if (my_ip != "1235"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                avg = 14.2362;//2;//14.2362;//14.2362;//35.253;//+36;
                std = 36.0803;//99.915;//36.0803;//36.0803;//2419.74;
                median = 3;//12;//0;
                if(num_outgoing[i] <= median)//pow(std,2))
                    if(num_outgoing[i] == 0)
                        weight = sqrt(avg);//num_outgoing[i]+1);
                    else
                        weight = sqrt(avg);// - (median - num_outgoing[i]) * (std / 2));
                else{
                    z_score = num_outgoing[i]/avg;//(num_outgoing[i]-avg)/std;
                    //if(z_score > 1)
                    weight = sqrt(z_score*avg);//num_outgoing[i] * sqrt((num_outgoing[i]-median)));//sqrt((num_outgoing[i]+1)+(num_outgoing[i] - median));//num_outgoing[i]-median));
                    //else
