    ...
    if (my_ip != "192.168.0.102"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                if(num_outgoing[i] <= 60)//pow(std,2))
                    weight = 1;//sqrt(num_outgoing[i]);// - (median - num_outgoing[i]) * (std / 2));
                else{
                    //z_score = ((num_outgoing[i]-round(avg))*(1.0/floor(std)));//avg)/std;
                    //if(z_score > 1)
                    weight = sqrt((num_outgoing[i]-60));//num_outgoing[i]);//((num_outgoing[i] * sqrt(z_score)) * sizeof(size_t)));//num_outgoing[i] * sqrt((num_outgoing[i]-median)));//sqrt((num_outgoing[i]+1)+(num_outgoing[i] - median));//num_outgoing[i]-median));
                    //else
                        //weight = sqrt(num_outgoing[i]);
                }
