    ...
    if (my_ip != "192.168.0.102"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                weight = ReLU(stod(alpha1), sqrt((num_outgoing[i]-round(avg)) * sizeof(size_t)));// + stod(alpha1);//ReLU(0 , sqrt((num_outgoing[i]-round(avg))*sizeof(size_t))) + sizeof(size_t);//sqrt(num_outgoing[i]);
