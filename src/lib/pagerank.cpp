        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                if(num_outgoing[i] <= avg)//pow(std,2))
                    ...
                    z_score = sqrt((num_outgoing[i]-avg)/std);
                    //if(z_score > 1)
                    weight = sqrt(z_score*sqrt(avg));//num_outgoing[i] * sqrt((num_outgoing[i]-median)));//sqrt((num_outgoing[i]+1)+(num_outgoing[i] - median));//num_outgoing[i]-median));
                    //else
