    ...
    if (my_ip != "192.168.0.102"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                avg = 14.2326;
                max = 20293;
                weight = 1 + (sqrt(num_outgoing[i]+(num_outgoing[i]-round(avg)))-1)*unit_step_func(num_outgoing[i]-avg);// + sqrt(num_outgoing[i]+avg)*unit_step_func(0.95 - num_outgoing[i]/max);
            }
            else{
