                                ...
                                int* recvcounts,vector<double> *send, vector<double> *recv1)
{
    ...
    if (my_ip != "1235"){
        ...
        for(int i =0; i<num_vertex;i++){
            double weight = 1;//sqrt(num_outgoing[i]+1.0);//sqrt(sqrt(pow(num_outgoing[i],2.5)) + 1.0);//sqrt(sqrt(pow(num_outgoing[i],2.7)) + 1.0);// / max_edge;//log10(static_cast<long double>(max_edge));//1+log(static_cast<long double>(num_outgoing[i]+1.0)); // 로그에 1을 더하여 0으로 나누는 오류를 피합니다.
