    ...
    if (my_ip != "1235"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                weight = pow(num_outgoing[i],1/1.7);//sqrt(num_outgoing[i]);
            else if(cmd == "3")
                weight = log(num_outgoing[i]+1.0);
            else if(cmd == "4")
                weight = pow(num_outgoing[i]+1.0,1/3.8);//sqrt(num_outgoing[i]+6.0);
            else if(cmd == "5")
                weight = log(num_outgoing[i]+(2.71828)+2.0);
