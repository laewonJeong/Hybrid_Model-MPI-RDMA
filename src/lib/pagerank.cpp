    ...
    if (my_ip != "192.168.0.102"){
        ...
        for(int i =0; i<num_vertex;i++){
            ...
            if(cmd == "1")
                ...
                avg =20;//35.253;// 27.528;//35.253;//14.2362;//35.253;//14.2362;//15.9151;//2;//15.9151;//6.54044;//5.57058;//11.092;//35.253;//14.2362;//35.253;//35.253;//2;//35.253;//14.2362;//14.2362;//2;//14.2362;//14.2362;//35.253;//+36;
                ...
                if(num_outgoing[i] <= 20)//pow(std,2)
                    ...
                    //z_score = ((num_outgoing[i]-20)*(1.0/floor(std)));//avg)/std;
                    //if(z_score > 1)
                    weight = sqrt(num_outgoing[i]*std);//num_outgoing[i]);//((num_outgoing[i] * sqrt(z_score)) * sizeof(size_t)));//num_outgoing[i] * sqrt((num_outgoing[i]-median)));//sqrt((num_outgoing[i]+1)+(num_outgoing[i] - median));//num_outgoing[i]-median));
                    //else
                        //weight = sqrt(num_outgoing[i]);
                }
