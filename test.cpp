...
//==================================================================================
    ...
    for(int i =0; i<num_of_vertex;i++){
        long double weight = (num_outgoing[i]+1)/(edge/num_of_vertex);//2*log2(static_cast<long double>(num_outgoing[i]+1.0))+2;// / max_edge;//log10(static_cast<long double>(max_edge));//1+log(static_cast<long double>(num_outgoing[i]+1.0)); // 로그에 1을 더하여 0으로 나누는 오류를 피합니다.
