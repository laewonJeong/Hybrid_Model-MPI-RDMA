...
int main(int argc, char** argv){
    ...
    int vertex_part = ceil(num_of_vertex/(num_of_node-1))*2;
    //cout << edge_part << endl;
    ...
    for(size_t i=0;i<num_of_vertex;i++){
        temp += num_outgoing[i];
        if(temp + ttt+ (ttt*sizeof(double)) > edge_part + vertex_part+buf_part){//+ ttt + (ttt*sizeof(double))> edge_part+vertex_part+buf_part){
